...
@switch($icon)
    @case('sparkle')
        <svg {{ $attributes->merge(['class' => 'h-3 w-3']) }} fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
    ...
    @case('heel')
        <svg {{ $attributes->merge(['class' => 'h-3 w-3']) }} fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6-6 3 3 6.75-6.75M19.5 18.75h-15"/>
        </svg>
        @break
    @case('bag')
        <svg {{ $attributes->merge(['class' => 'h-3 w-3']) }} fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
